...
use crate::constants::physics_constants::{GRAVITY, SOLVER_TIMESTEP};
use std::arch::asm;
...
fn euler_integration(angle: f64, velocity: f64, mut x1: f64, mut y1: f64, x2: f64, y2: f64) -> f64 {
    ...
    while (x1 < x2) && (y1 >= 0.0) {
        ...
        x_velocity -= dx_velocity * SOLVER_TIMESTEP;
        y_velocity -= dy_velocity * SOLVER_TIMESTEP;

        x1 += x_velocity * SOLVER_TIMESTEP;
        y1 += y_velocity * SOLVER_TIMESTEP;
    }
    ...
    while (x1 < x2) && (y1 >= 0.0) {
        unsafe {
            asm!(
                ...
                "vfmadd132sd {1}, {2}, {5}", // x_velocity -= dx_velocity * SOLVER_TIMESTEP
                "vfmadd231sd {0}, {1}, {4}", // x1 += x_velocity * SOLVER_TIMESTEP

                inout(xmm_reg) x1 => x1, // 0
                inout(xmm_reg) x_velocity => x_velocity, // 1
                in(xmm_reg) x_velocity, // 2
                in(xmm_reg) x_drag_factor, // 3
                in(xmm_reg) SOLVER_TIMESTEP, // 4
                in(xmm_reg) -SOLVER_TIMESTEP // 5
            );
            ...
            asm!(
                ...
                "vfmadd132sd {1}, {2}, {6}", // y_velocity -= dy_velocity * SOLVER_TIMESTEP
                "vfmadd231sd {0}, {1}, {5}", // y1 += y_velocity * SOLVER_TIMESTEP
...
pub fn integrate(angle: f64, velocity: f64, x1: f64, y1: f64, x2: f64, y2: f64) -> f64 {
    euler_integration(angle, velocity, x1, y1, x2, y2)
}
